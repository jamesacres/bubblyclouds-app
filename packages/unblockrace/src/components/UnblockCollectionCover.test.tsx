...
import UnblockCollectionCover from './UnblockCollectionCover';

describe('UnblockCollectionCover', () => {
  it('renders the month label in portrait variant by default', () => {
    render(<UnblockCollectionCover month="July 2026" />);
  ...
  it('applies medium size dimensions by default', () => {
    const { container } = render(<UnblockCollectionCover month="July 2026" />);
  ...
  it.each([
    ...
    const { container } = render(
      <UnblockCollectionCover month="July 2026" size={size} />
    );
    const cover = container.firstElementChild as HTMLElement;
  ...
  it('renders the tile variant without the portrait-only copy', () => {
    render(<UnblockCollectionCover month="July 2026" variant="tile" />);
    expect(screen.getByText('July 2026')).toBeInTheDocument();
  ...
  it('renders a board preview in both variants', () => {
    const { container: portrait } = render(
      <UnblockCollectionCover month="July 2026" variant="portrait" />
    );
    const { container: tile } = render(
      <UnblockCollectionCover month="July 2026" variant="tile" />
    );
