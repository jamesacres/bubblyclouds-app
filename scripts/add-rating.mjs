...
export function removeDiacritics(input) {
  ...
  const diacriticsMap = {};
  for (let i = 0; i < defaultDiacriticsRemovalMap.length; i = i + 1) {
    const letters = defaultDiacriticsRemovalMap[i].letters;
    for (let j = 0; j < letters.length; j = j + 1) {
...
export function slugify(input) {
  return removeDiacritics(input.toString())
    ...
    .replace(/[^\w\-]/g, '_'); // Replace all non-word chars with _
}

const { artist } = await enquirer.prompt({
  type: 'input',
  name: 'artist',
...
const { rating: ratingOutput } = await new Scale({
  name: 'rating',
  message: 'What is your rating?',
  scale: [
    { name: '0.5', message: '0.5' },
    { name: '1', message: '1' },
    { name: '1.5', message: '1.5' },
    { name: '2', message: '2' },
    { name: '2.5', message: '2.5' },
    { name: '3', message: '3' },
    { name: '3.5', message: '3.5' },
    { name: '4', message: '4' },
    { name: '4.5', message: '4.5' },
    { name: '5', message: '5' },
  ],
  margin: [0, 0, 2, 1],
  choices: [
...
const ratingOutOfTen = ratingOutput + 1;
...
const blogPath = `apps/stephenesch/data/blog/${key}.mdx`;
const blogContent = `---
