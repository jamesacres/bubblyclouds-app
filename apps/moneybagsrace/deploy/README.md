# Money Bags Race Deploy

AWS CDK stack that creates the infrastructure for deploying the Money Bags Race
...
- **CloudFront Function** - Rewrites URLs (e.g. `/state` to `/state.html`) for
...
After building the app (`pnpm run build:moneybagsrace` from the repo root), sync
