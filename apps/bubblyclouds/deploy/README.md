...
AWS CDK stack that creates the infrastructure for deploying the Bubblyclouds
website as a fully static site.

## Infrastructure

- **S3 Bucket** - Hosts the static build output
- **CloudFront Distribution** - CDN with custom domain and SSL certificate
- **CloudFront Function** - Rewrites URLs (e.g. `/about` to `/about.html`) for
...
After building the app (`pnpm run build:bubblyclouds` from the repo root), sync
