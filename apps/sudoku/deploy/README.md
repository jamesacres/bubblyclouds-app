...
AWS CDK stack that creates the infrastructure for deploying the Sudoku web app
...
After building the app (`pnpm run build:sudoku` from the repo root), sync the
static output to S3 and invalidate the CloudFront cache:
