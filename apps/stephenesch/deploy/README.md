...
AWS CDK stack that creates the infrastructure for deploying the Stephen Esch blog
...
- **CloudFront Function** - Rewrites URLs (e.g. `/blog/post` to
  `/blog/post.html`) for clean Next.js static export routes
...
After building the app (`pnpm run build:stephenesch` from the repo root), sync
